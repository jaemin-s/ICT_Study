...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testList() throws Exception {
		...
		System.out.println("date in Model: "+mv.getModelMap());
	...
	void testContent() throws Exception {
		ModelAndView mv = mockMvc.perform(MockMvcRequestBuilders.get("/freeboard/content").param("bno", "3"))
				.andReturn().getModelAndView();
		
		System.out.println("content: "+mv.getModelMap());
		System.out.println("viewName: "+mv.getViewName());
	}
	
	@Test
	@DisplayName("3번글의 제목과 내용을 수정하는 요청을 post방식으로 전송하면 수정이 진행되고 수정된 글의 상세보기 페이지로 이동할 것이다.")
	// /freeboard/modify -> post
	void testModify() throws Exception {
		int bno = 3;
		ModelAndView mv = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")
				.param("bno", Integer.toString(bno))
				.param("title", "바뀐 글 제목")
				.param("content", "바뀐 글 내용"))
				.andReturn().getModelAndView();
