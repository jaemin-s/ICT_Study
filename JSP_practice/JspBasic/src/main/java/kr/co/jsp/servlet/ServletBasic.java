...
/*
 ...
 톰캣을 사용하여 jsp파일을 자동으로 class로 변환했다면
...
//		PrintWriter 객체를 사용합니다.
		...
		htmlCode += "<!DOCTYPE html>\r\n"
                ...
                + "<body>\r\n"
                + "\r\n"
                + "\t아이디: " + req.getParameter("account") + "\r\n"
                + "\t이메일: " + req.getParameter("email") + "\r\n"
                + "\r\n"
                + "</body>\r\n"
                + "</html>\r\n"
                + "";
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// get 요청이 발생될때 호출되는 메서드 
		System.out.println("doGet 호출 됨");
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//http 통신 중 post 요청이 발생될때 자동으로 호출되는 메서드
		System.out.println("doPost 호출 됨");
	}

	@Override
	public void destroy() {
		// 서블릿 객체가 소멸될 때 호출되는 메서드
